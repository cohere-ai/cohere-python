...
class TestGenerate(unittest.TestCase):
    def test_success(self):
        prediction = co.generate(model="command-light", prompt="co:here", max_tokens=1)
    ...
    def test_success_batched(self):
        _batch_size = 10
        predictions = co.batch_generate(model="command-light", prompts=["co:here"] * _batch_size, max_tokens=1)
    ...
    def test_batch_return_exceptions(self):
        ...
        with self.assertRaises(cohere.CohereError):
            predictions = co.batch_generate(model="command-light", prompts=prompts, max_tokens=1)
        # test return_exceptions = True
        predictions = co.batch_generate(model="command-light", prompts=prompts, max_tokens=1, return_exceptions=True)
    ...
    def test_return_likelihoods_generation(self):
        prediction = co.generate(model="command-light", prompt="co:here", max_tokens=1, return_likelihoods="GENERATION")
    ...
    def test_return_likelihoods_all(self):
        prediction = co.generate(model="command-light", prompt="hi", max_tokens=1, return_likelihoods="ALL")
    ...
    def test_invalid_temp(self):
        with self.assertRaises(cohere.CohereError):
            co.generate(model="command-light", prompt="hi", max_tokens=1, temperature=-1).generations
    ...
    def test_no_version_works(self):
        cohere.Client(API_KEY).generate(model="command-light", prompt="co:here", max_tokens=1).generations

...
def test_consistency_and_docs():
    ...
    for name, func in client_methods.items():
        ...
        if not name.startswith("batch_"):  # these have return_exceptions
            ...
            if (
                client_signature.return_annotation != async_signature.return_annotation
            ):  # some methods return a separate async class
                assert (
                    "Async" + client_signature.return_annotation.__name__ == async_signature.return_annotation.__name__
                )
